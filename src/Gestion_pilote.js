document.addEventListener("DOMContentLoaded", () => {
    ...
    const API_URL = 'http://localhost/3Dsite%20version%201.3/api_gestion_pilote.php';
    ...
    const loadStudents = async (page = 1, limit = studentsPerPage) => {
        try {
            ...
            console.error(`Erreur lors du chargement des pilotes : ${error.message}`);
            statsContainer.innerHTML = `<div class="error-message">Erreur lors du chargement des pilotes</div>`;
    ...
    const getStudentById = async (id) => {
        try {
            ...
            console.error(`Erreur lors du chargement du pilote : ${error.message}`);
            return null;
        }
    };
    ...
    const createStudent = async (studentData) => {
        try {
            ...
            if (result.success) {
                ...
                alert(`Compte pilote créé avec succès pour ${studentData.prenom} ${studentData.nom}`);
    ...
    const updateStudent = async (studentData) => {
        try {
            ...
            if (result.success) {
                ...
                alert(`Compte pilote mis à jour avec succès pour ${studentData.prenom} ${studentData.nom}`);
    ...
    const deleteStudent = async (id) => {
        try {
            ...
            if (result.success) {
                ...
                alert(`Compte pilote supprimé avec succès`);
    ...
    const populateStudentSelect = async (selectElement) => {
        try {
            ...
            if (result.success && result.data.length > 0) {
                selectElement.innerHTML = '<option value="">Sélectionnez un pilote</option>';
                ...
                throw new Error(result.error || 'Aucun pilote trouvé');
            }
        } catch (error) {
            console.error(`Erreur lors du chargement des pilotes : ${error.message}`);
    ...
    const loadStudentForEdit = async (id) => {
        try {
            ...
            if (student) {
                ...
                throw new Error('pilote non trouvé');
            }
        } catch (error) {
    ...
    const displayStudentsList = (students, pagination) => {
        ...
        if (students.length === 0) {
            ...
            noDataMsg.textContent = 'Aucun pilote trouvé.';
    ...
    const openModal = (title, type) => {
        ...
        if (type === "chercher") {
            ...
            form.innerHTML = `
                <label for="student-select">Sélectionnez un pilote :</label>
            ...
            form.innerHTML = `
                <label for="delete-student-select">Sélectionnez un pilote à supprimer :</label>
                ...
                <div id="confirmation" style="display: none; margin-top: 15px;">
                    ...
                    <p style="font-family: 'NoitaBlackletter', serif; text-align: center;">
                        Êtes-vous sûr de vouloir supprimer cet pilote ?
                    </p>
    ...
    const displaySearchResults = (students) => {
        ...
        if (students.length === 0) {
            ...
            noResults.textContent = 'Aucun pilote trouvé.';
    ...
    form.addEventListener("submit", async (event) => {
        ...
        if (action.includes("Chercher")) {
            ...
            if (!selectedStudent || !selectedStudent.Id_Utilisateur) {
                displayErrorMessage("Veuillez sélectionner un pilote");
                return;
            }
    ...
    document.getElementById("btn-chercher")?.addEventListener("click", () => openModal("Chercher un pilote", "chercher"));
    document.getElementById("btn-supprimer")?.addEventListener("click", () => openModal("Supprimer un pilote", "supprimer"));
    document.getElementById("btn-creer")?.addEventListener("click", () => openModal("Créer un pilote", "creer"));
    document.getElementById("btn-modifier")?.addEventListener("click", () => openModal("Modifier un pilote", "modifier"));
